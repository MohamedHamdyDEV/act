...
submit.addEventListener("click", () => {
  ...
  let hdd = new int(document.getElementById("date-1").value);
  ...
  //if (date1.getTime() && date2.getTime()) {
    ...
    output.innerHTML = `Activation Code IS <span>${dayDifference}</span> `;
  }

  //Else display that the input is valid
  else {
    output.innerHTML = "Please Input A Valid HDD Serial";
  }
});
